...
powershell -NoProfile -Command "try { (Invoke-WebRequest -UseBasicParsing -TimeoutSec 5 http://127.0.0.1:%PORT%/api/health).StatusCode } catch { exit 1 }" >nul 2>&1
if errorlevel 1 (
  ...
  if not errorlevel 1 (
    ...
    set /a WAIT+=1
    if !WAIT! LSS 30 goto :waitbusy
    echo DNT Dental server is busy or not responding. Check %DATA_DIR%\logs
    pause
    exit /b 1
  )
